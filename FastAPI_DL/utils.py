...
def insert_segmentation_results_db(conn, task_id, segmentation_polygons, user_id, status, table_name="mark", batch_size=1000):
    ...
    try:
        with conn.cursor() as cursor:
            # 将多边形数据格式化为插入值
            values_list = [
                (', '.join([f"{x}, {y}" for x, y in polygon.exterior.coords]), 
                 int(type_id), user_id, task_id, status)
...
# 数据集定义
# def calculate_window_from_polygons(polygons, transform, img_height, img_width):
#     """根据多边形范围计算 Rasterio 窗口"""
#     if polygons is None:
#         return Window(0, 0, img_width, img_height)  # 整张影像的窗口
...
#     col_stop = min(img_width, int(window.col_off + window.width))
#     row_stop = min(img_height, int(window.row_off + window.height))
...
#             model_scope_str: 模型作用范围（JSON字符串）
#             apply_transforms: 是否应用数据增强（仅训练时使用）
#         """
...
#                 print(f"警告: 图像坐标系为 {self.crs}，不是 EPSG:3857，可能导致掩膜生成错误！")

#         # 计算窗口
#         self.window = calculate_window_from_polygons(self.model_scope_polygons, self.transform, 
...
#         # 数据增强变换（仅训练时应用）
#         if self.apply_transforms and self.labels_data is not None:
#             self.transform_pipeline = T.Compose([
#                 T.RandomHorizontalFlip(p=0.5),
#                 T.RandomVerticalFlip(p=0.5),
#                 T.RandomRotation(degrees=(-45, 45), fill=(self.background_class_index,)),
#             ])
#         else:
...
#             # 读取窗口内的影像数据
#             image = src.read(window=self.window)
#             image = image.astype(np.float32) / 255.0

#             # 如果有标签数据，生成窗口内的掩膜
#             if self.labels_data is not None and self.num_classes is not None:
#                 # 调整变换矩阵以匹配窗口
#                 window_transform = src.window_transform(self.window)
#                 label_mask = self._create_label_mask(self.labels_data, window_transform, 
...
#             scope_mask = create_scope_mask(self.model_scope_polygons, self.transform, 
#                                          self.img_height, self.img_width)
#             # 提取窗口部分的 scope_mask
#             scope_mask = scope_mask[self.window.row_off:self.window.row_off + self.window.height,
#                                    self.window.col_off:self.window.col_off + self.window.width]
#             image, label_mask = apply_scope_to_image_and_mask(image, label_mask, scope_mask)

#         return image, label_mask

#     def _create_label_mask(self, labels_data, transform, height, width):
#         """创建窗口内的标签掩膜"""
#         mask = np.full((height, width), self.background_class_index, dtype=np.uint8)
#         shapes = []
...
#         """获取窗口化的影像和掩膜数据"""
#         image, label_mask = self._load_window_data()
#         if image is None:
#             raise ValueError("影像数据为 None，无法继续推理。")
#         # 如果是训练阶段且需要掩膜，则检查 label_mask
#         if self.apply_transforms and self.labels_data is not None and label_mask is None:
#             raise ValueError("掩膜数据为 None，且需要用于训练。")

#         # if image is None or (label_mask is None and self.labels_data is not None):
#         #     raise ValueError("影像或掩膜数据为 None。")

#         image = torch.from_numpy(image).float()
#         label_mask = torch.from_numpy(label_mask).long() if label_mask is not None else None
...
#         return image, label_mask, self.window_transform
